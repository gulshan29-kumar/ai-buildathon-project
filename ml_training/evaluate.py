...
def compute_metrics(y_true: np.ndarray, y_prob: np.ndarray, threshold: float = 0.5) -> Dict[str, Any]:
    """Computes all required evaluation metrics for binary classification."""
    ...
    # Calibration bins (fraction of positives vs mean predicted probability)
    try:
...
def evaluate_model(
    ...
    print("=" * 65)
    print(f"  Samples:      {global_metrics['sample_count']:,} (Recovered: {global_metrics['recovered_count']:,})")
    ...
    print("=" * 65)
    print(f"Detailed evaluation report saved to: {report_output}\n")
